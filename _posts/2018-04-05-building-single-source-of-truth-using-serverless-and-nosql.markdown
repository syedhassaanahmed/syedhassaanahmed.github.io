...
The above script uses containerized version of [wrk2](https://github.com/giltene/wrk2) *(wrk2 is wrk modified to produce a constant throughput load)*. `WRK_OPTIONS` specified above, make sure the benchmark runs for 2 minutes, using a single thread, keeping 100 HTTP connections open, with a throughput of ~1500 requests per second. In addition to `TARGET_URL` the container also takes `SCRIPT_URL` as an environment variable, which points to a [LuaJIT](http://luajit.org/) script. Script allows us to generate dynamic HTTP request body;
```lua
wrk.method = "POST"
